...
export default function ManageRetailersPage() {
  ...
  const { data: retailers = [], isLoading } = useQuery({
    queryKey: ['users', 'RETAILER'],
    queryFn: () => usersAPI.getByRole('RETAILER').then((r) => r.data),
  });
  ...
  return (
    <div className="page active">
      ...
      <div className="table-card">
        <div className="tbar">
          <span className="panel-title">Retailer accounts (created by Admin)</span>
        </div>
        {isLoading ? (
          ...
          <div className="p-6" style={{ color: 'var(--text-muted)' }}>
            No retailers yet. Create one from Create Retailer.
          </div>
        ) : (

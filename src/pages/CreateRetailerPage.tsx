...
export default function CreateRetailerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to create retailer');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="page active">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text-white)' }}>Create Retailer</h1>
      </div>

      <div className="panel" style={{ maxWidth: 480 }}>
        <div className="panel-hdr">
          <span className="panel-title flex items-center gap-2">
            <Store className="w-5 h-5" /> New Retailer Account
          </span>
        </div>
        <div className="panel-body">
          {success && (
            <div className="mb-4 p-3 rounded-lg flex items-center gap-2" style={{ background: 'var(--green-bg)', color: 'var(--green)', border: '1px solid var(--green-border)' }}>
              Retailer created successfully. They can now log in with the provided email and password.
            </div>
          )}
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="field">
              <label className="label" htmlFor="retailer-name">Name</label>
              <input
                id="retailer-name"
                type="text"
                className="input"
            ...
            <div className="field">
              <label className="label" htmlFor="retailer-email">Email</label>
              <input
                id="retailer-email"
                type="email"
                className="input"
            ...
            <div className="field">
              <label className="label" htmlFor="retailer-password">Password</label>
              <input
            ...
            <div className="flex gap-2">
              ...
              <button type="button" className="tb-btn" onClick={() => navigate('/manage-retailers')}>
                View Retailers
              </button>
            </div>
          </form>
